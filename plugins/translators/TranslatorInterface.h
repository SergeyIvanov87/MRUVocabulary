...
struct ITranslatorPlugin// : virtual public IPlugin
{
    ...
    virtual long long translate(PluginCtxPtr &translator_ctx, SharedDecodedData &in_decoder_session, SharedCtxPtr& out_translator_session) = 0;

    using cstr_ptr = std::unique_ptr<char, decltype(&free)>;
    virtual cstr_ptr sharedCtx2CStr(PluginCtxPtr &translator_ctx, SharedCtxPtr& in_translator_session) const = 0;
};
